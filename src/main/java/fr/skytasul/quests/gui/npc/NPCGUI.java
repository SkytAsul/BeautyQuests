...
public class NPCGUI implements CustomInventory{
	...
	public void onClick(Player p, Inventory inv, ItemStack current, int slot, ClickType click){
		...
		case 3:
			...
			/*Utils.openTchatEditor(p, inv, null, (str) -> {
					if (str != null) inv.setItem(slot, ItemUtils.skull(ItemUtils.getName(skin), (String) str, ItemUtils.getLore(skin)));
			});*/
			break;
		...
		case 7:
			...
			Inventories.create(p, new EntityTypeGUI((obj) -> {
					...
					if (en == EntityType.PLAYER){
						inv.setItem(7, ItemUtils.skull(Lang.name.toString(), null, "player"));
					}else inv.setItem(7, ItemUtils.item(XMaterial.mobItem(en), Lang.name.toString(), en.getName()));
			}));
			break;
