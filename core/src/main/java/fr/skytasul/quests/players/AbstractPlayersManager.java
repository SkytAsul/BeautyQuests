...
public abstract class AbstractPlayersManager implements PlayersManager {
	...
	public synchronized void loadPlayer(@NotNull Player p) {
		...
		BeautyQuests.getInstance().getScheduler().runTask(SchedulerType.ASYNC, schedulerTaskInter -> {
			for (int i = 1; i >= 0; i--) {
				try {
					if (!tryLoad(p, time))
						return;
	...
	private boolean tryLoad(@NotNull Player p, long time) {
		...
		BeautyQuests.getInstance().getScheduler().runTask(SchedulerType.SYNC, p, taskInter -> {
			String loadMessage =
