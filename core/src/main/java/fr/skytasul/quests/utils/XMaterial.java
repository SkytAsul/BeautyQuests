...
public enum XMaterial {
	 ...
	 * @since 3.0.0
	 */
	private static final ImmutableMap<XMaterial, XMaterial> duplicated = Maps.immutableEnumMap(ImmutableMap.<XMaterial, XMaterial>builder().put(MELON, MELON_SLICE).put(CARROT, CARROTS).put(POTATO, POTATOES).put(BEETROOT, BEETROOTS).put(BROWN_MUSHROOM, BROWN_MUSHROOM_BLOCK).put(BRICK, BRICKS).put(NETHER_BRICK, NETHER_BRICKS)
			...
			.put(DARK_OAK_DOOR, DARK_OAK_DOOR).put(ACACIA_DOOR, ACACIA_DOOR).put(BIRCH_DOOR, BIRCH_DOOR).put(JUNGLE_DOOR, JUNGLE_DOOR).put(SPRUCE_DOOR, SPRUCE_DOOR)
	...
	*/
	...
	public static XMaterial matchXMaterial(Material material) {
		...
		return matchDefinedXMaterial(material.name(), (byte) -1).orElseThrow(() -> new IllegalArgumentException("Unsupported Material With No Bytes: " + material.name()));
	}
	...
	public static XMaterial mobItem(EntityType type) {
		...
		if (type == EntityType.IRON_GOLEM) return IRON_BLOCK;
		if (type == EntityType.SNOWMAN) return SNOW_BLOCK;
		if (type == EntityType.MUSHROOM_COW) return MOOSHROOM_SPAWN_EGG;
		if (type == EntityType.GIANT) return ZOMBIE_SPAWN_EGG;
		...
		if (type.name().equals("PIG_ZOMBIE") || type.name().equals("ZOMBIFIED_PIGLIN")) return ZOMBIE_PIGMAN_SPAWN_EGG;
		return SPONGE;
	}
